...
require 'optim'
function optim.sgd_log(opfunc, x, config, state)
   -- (0) get/update state
   ...
   if lrs then
	  ...
	  --store magnitude of update
	  state.update_norm = torch.mul(state.deltaParameters,clr):norm()

      x:add(-clr, state.deltaParameters)
   else
	  state.update_norm = torch.mul(dfdx,clr):norm()
      x:add(-clr, dfdx)
   end
